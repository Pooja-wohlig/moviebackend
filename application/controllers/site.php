...
//-------------------------------------------MOVIE_WATCH---------------------------------------------------------------------------------------
	 ...
	 public function viewwatch()
    {
        ...
        $this->checkaccess($access);
        $data["page"]="viewwatch";
		$data['page2']='block/watchblock';
        $data['movieid']=$this->input->get('id');
		$data['before']=$this->movie_model->beforeedit($this->input->get('id'));
        $movieid=$this->input->get('id');
//		$data['before']=$this->watch_model->beforeedit($this->input->get('id'));
        $data["base_url"]=site_url("site/viewwatchjson?id=".$movieid);
        $data["title"]="View watch";
        $this->load->view("templatewith2",$data);
    }
    function viewwatchjson()
    {
        $movieid=$this->input->get('id');
        $elements=array();
        
        $elements[0]=new stdClass();
        $elements[0]->field="`movie_watch`.`id`";
        $elements[0]->sort="1";
        $elements[0]->header="ID";
        $elements[0]->alias="id";
        
        $elements[1]=new stdClass();
        $elements[1]->field="`movie_watch`.`user`";
        $elements[1]->sort="1";
        $elements[1]->header="User";
        $elements[1]->alias="user";
        
        $elements[2]=new stdClass();
        $elements[2]->field="`movie_watch`.`movie`";
        $elements[2]->sort="1";
        $elements[2]->header="Movie";
        $elements[2]->alias="movie";
        
        $elements[3]=new stdClass();
        $elements[3]->field="`movie_watch`.`status`";
        $elements[3]->sort="1";
        $elements[3]->header="Status";
        $elements[3]->alias="status";
		...
		$elements[5]=new stdClass();
        $elements[5]->field="`movie_movie`.`name`";
        $elements[5]->sort="1";
        $elements[5]->header="Moviename";
        $elements[5]->alias="moviename";
        ...
        $data["message"]=$this->chintantable->query($pageno,$maxrow,$orderby,$orderorder,$search,$elements,"FROM `movie_watch` LEFT OUTER JOIN `movie_movie` ON `movie_movie`.`id`=`movie_watch`.`movie` LEFT OUTER JOIN `user` ON `user`.`id`=`movie_watch`.`user`","WHERE `movie_watch`.`movie`='$movieid'");
        $this->load->view("json",$data);
    }
    ...
    public function createwatch()
    {
        ...
        $data['movie']=$this->movie_model->getmoviedropdown();
        $data['status']=$this->movie_model->getstatusdropdown();
        $data['user']=$this->movie_model->getuserdropdown();
        $data['movieid']=$this->input->get('id');
        $data["page"]="createwatch";
        $data["title"]="Create watch";
        $this->load->view("template",$data);
    }
    public function createwatchsubmit() 
    {
        $access=array("1");
        $this->checkaccess($access);
        $this->form_validation->set_rules("user","User","trim");
        ...
        else
        {
            ...
            $movie=$this->input->get_post("movie");			
            $status=$this->input->get_post("status");
            if($this->watch_model->create($user,$movie,$status)==0)
                $data["alerterror"]="New watch could not be created.";
            else
                $data["alertsuccess"]="watch created Successfully.";
            $data["redirect"]="site/viewwatch?id=".$movie;
            $this->load->view("redirect2",$data);
        }
    }
    public function editwatch()
    {
        ...
        $this->checkaccess($access);
        $data["page"]="editwatch";
        $data["title"]="Edit watch";
        $data['movie']=$this->movie_model->getmoviedropdown();
        $data['status']=$this->movie_model->getstatusdropdown();
		$data['user']=$this->movie_model->getuserdropdown();
        $data['movieid']=$this->input->get('id');
        $data['watchid']=$this->input->get('watchid');
        $data["before"]=$this->watch_model->beforeedit($this->input->get("watchid"));
        $this->load->view("template",$data);
    }
    public function editwatchsubmit()
    {
        $access=array("1");
        $this->checkaccess($access);
        $this->form_validation->set_rules("id","ID","trim");
        ...
        if($this->form_validation->run()==FALSE)
        {
			...
			$data['user']=$this->movie_model->getuserdropdown();
            $data['movieid']=$this->input->post('movie');
            $data['watchid']=$this->input->post('id');
            $data["before"]=$this->watch_model->beforeedit($this->input->post("id"));
            $this->load->view("template",$data);
        }
        else
        {
            ...
            $movie=$this->input->get_post("movie");
            $status=$this->input->get_post("status");
            if($this->watch_model->edit($id,$user,$movie,$status)==0)
                $data["alerterror"]="New watch could not be Updated.";
            else
                $data["alertsuccess"]="Watch Updated Successfully.";
            $data["redirect"]="site/viewwatch?id=".$movie;
            $this->load->view("redirect2",$data);
        }
    }
    public function deletewatch()
    {
        ...
        $data['movieid']=$this->input->get('id');
        $data['watchid']=$this->input->get('watchid');
        $this->watch_model->delete($this->input->get("watchid"));
        $data["redirect"]="site/viewwatch?id=".$this->input->get('id');
        $this->load->view("redirect2",$data);
...
//	---------------------------------------------------REVIEWS------------------------------------------------------------------------------
	   public function viewreview()
    {
        ...
        $movieid=$this->input->get('id');
//		$data['before']=$this->watch_model->beforeedit($this->input->get('id'));
        $data["base_url"]=site_url("site/viewreviewjson?id=".$movieid);
    ...
    function viewreviewjson()
    {
        ...
        $elements[0]=new stdClass();
        $elements[0]->field="`movie_review`.`id`";
        $elements[0]->sort="1";
        $elements[0]->header="ID";
        $elements[0]->alias="id";
        
        $elements[1]=new stdClass();
        $elements[1]->field="`movie_review`.`user`";
        $elements[1]->sort="1";
        $elements[1]->header="User";
        $elements[1]->alias="user";
        
        $elements[2]=new stdClass();
        $elements[2]->field="`movie_review`.`movie`";
        $elements[2]->sort="1";
        $elements[2]->header="Movie";
        $elements[2]->alias="movie";
        
        $elements[3]=new stdClass();
        $elements[3]->field="`movie_review`.`status`";
        $elements[3]->sort="1";
        $elements[3]->header="Status";
        $elements[3]->alias="status";
		
		$elements[4]=new stdClass();
        $elements[4]->field="`movie_review`.`review`";
        $elements[4]->sort="1";
        $elements[4]->header="Review";
        $elements[4]->alias="review";
		...
		$elements[6]=new stdClass();
        $elements[6]->field="`movie_movie`.`name`";
        $elements[6]->sort="1";
        $elements[6]->header="Moviename";
        $elements[6]->alias="moviename";
        ...
        $data["message"]=$this->chintantable->query($pageno,$maxrow,$orderby,$orderorder,$search,$elements,"FROM `movie_review` LEFT OUTER JOIN `movie_movie` ON `movie_movie`.`id`=`movie_review`.`movie` LEFT OUTER JOIN `user` ON `user`.`id`=`movie_review`.`user`","WHERE `movie_review`.`movie`='$movieid'");
        $this->load->view("json",$data);
    }
    ...
    public function createreviewsubmit() 
    {
        ...
        else
        {
            ...
            if($this->review_model->create($user,$movie,$status,$review)==0)
                $data["alerterror"]="New review could not be created.";
            else
                $data["alertsuccess"]="review created Successfully.";
            $data["redirect"]="site/viewreview?id=".$movie;
            $this->load->view("redirect2",$data);
        }
    }
    public function editreview()
    {
		...
		$data['user']=$this->movie_model->getuserdropdown();
        $data['movieid']=$this->input->get('id');
        $data['reviewid']=$this->input->get('reviewid');
        $data["before"]=$this->review_model->beforeedit($this->input->get("reviewid"));
		$temp=$data["before"]->review;
		echo $temp;
        $this->load->view("template",$data);
    }
    public function editreviewsubmit()
    {
        ...
        if($this->form_validation->run()==FALSE)
        {
			...
			$data['user']=$this->movie_model->getuserdropdown();
            $data['movieid']=$this->input->post('movie');
            $data['reviewid']=$this->input->post('id');
            $data["before"]=$this->review_model->beforeedit($this->input->post("id"));
            $this->load->view("template",$data);
        }
        else
        {
            ...
            $status=$this->input->get_post("status");
            $review=$this->input->get_post("review");
            if($this->review_model->edit($id,$user,$movie,$status,$review)==0)
    ...
    public function deletereview()
    {
        ...
        $data['movieid']=$this->input->get('id');
        $data['reviewid']=$this->input->get('reviewid');
        $this->review_model->delete($this->input->get("reviewid"));
        $data["redirect"]="site/viewreview?id=".$this->input->get('id');
        $this->load->view("redirect2",$data);
